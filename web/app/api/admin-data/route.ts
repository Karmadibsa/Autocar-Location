...
export async function POST(request: Request) {
  ...
  const [{ count: leads }, devisRes, recentRes, statutsRes, refusRes] = await Promise.all([
    ...
    sb
      .from("demandes")
      .select(
        "id, depart, destination, date_depart, aller_retour, distance_km, nb_passagers, urgence, commentaire, statut, created_at, clients(email, prenom, nom), devis(prix_ht, tva, prix_ttc, lignes, coefficients, statut, nb_relances, prochaine_relance, raison_refus)",
      )
